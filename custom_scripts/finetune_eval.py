...
# --- !!! UPDATE THIS PATH to point to your fine-tuned model's predictions !!! ---
PRED_XML_DIR = Path("/home/emrek/cursor/yoloe/yoloe/finetune_pred_dir_80epoch/annotations")
# Assume images are in a parallel directory
IMG_DIR      = Path("/home/emrek/cursor/yoloe/yoloe_test_dataset/JPEGImages")
# --- Use a different output directory for visualizations ---
VISUALIZATION_OUTPUT_DIR = Path("finetune_output_visualizations_80epoch")
...
for xml_gt in tqdm(sorted(GT_XML_DIR.glob("*.xml"))):
    ...
    try:
        ...
        print(f"Error updating metric for image {img_id}:")
...
try:
    ...
    print("--- Full Results Dictionary (COCO Metric) ---")
    print(coco_results)
    print("--- Full Results Dictionary (AP@0.50 Metric) ---")
    print(map50_results)
    ...
    if 'map_per_class' in coco_results:
        ...
        print("Per-class AP @ 0.5:0.95 not available in results.")
    ...
    if 'map_per_class' in map50_results:
        ...
        print("Per-class AP @ 0.50 not available in results (missing 'map_per_class' key in map50_results).")
