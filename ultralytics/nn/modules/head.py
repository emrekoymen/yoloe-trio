...
class YOLOEDetect(Detect):
    ...
    def fuse(self, txt_feats):
        ...
        for cls_head, bn_head in zip(self.cv3, self.cv4):
            ...
            # Restore original weight calculation logic
            w = conv.weight.data.squeeze(-1).squeeze(-1) # Original weights [512, 256]
            b = conv.bias.data # Original bias [512]
            
            # Ensure t has 3 dims for matmul: [N, 1, D] = [6, 1, 512]
            if t.ndim == 2:
                t = t.unsqueeze(1)
            assert t.shape[0] == self.nc and t.shape[2] == self.embed, f"Shape mismatch: t {t.shape} vs expected [{self.nc}, 1, {self.embed}]"
            assert w.shape == (self.embed, conv.in_channels), f"Shape mismatch: w {w.shape} vs expected [{self.embed}, {conv.in_channels}]"

            # Perform original matrix multiplication
            w = t @ w # [6, 1, 512] @ [512, 256] -> [6, 1, 256]

            # Original bias calculation
            b1 = (t @ b.reshape(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1) # [6, 1, 512] @ [512, 1] -> [6, 1, 1] -> squeeze -> [6]
            b2 = torch.ones_like(b1) * bias # [6]
            
            # Create new conv layer mapping C3 -> NC (256 -> 6)
            ...
            # Correctly reshape w ([6, 1, 256]) to [6, 256, 1, 1] for copying
            fused_conv.weight.data.copy_(w.squeeze(1).unsqueeze(-1).unsqueeze(-1))
            fused_conv.bias.data.copy_(b1 + b2) # Copy bias [6]
            cls_head[-1] = fused_conv # Replace final conv layer in the sequence
            
            bn_head.fuse() # Fuse the associated BNContrastiveHead (now redundant but part of original logic)
